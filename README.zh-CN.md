...
**[01 · 把模糊的现场问题，落成能执行的方案](#solve)**

- [存量合同档案的 LLM 辅助梳理与确权方案](#contracts)：把“一摞手写的陈年合同”理清成权属关系明确、可交律师复核的法律文书草案
- [小微餐饮档口的 AI 辅助开业方案](#stall)：只凭“一张照片和两段语音”，做出可施工、带验收清单的开业方案
- [IT 工单智能分诊与运营看板方案](#opssignal)：先定义工作再上 AI，84% 的工单自动分诊
- [B2B Agent 故障排查与客户沟通演练场](#proptech)：公开演练从客户反馈到根因修复、再到对外解释的全过程
- [智能服装制造设备的商业化落地](#aistar)：把 27 款智能设备从工厂推向客户产线

**[02 · 给 AI 划清边界，并证明它可信](#ai)**

- [企业 Agent 权限管控与审计内核](#acp)：把 Agent 的权限从提示词搬进代码，每一步都可审计
- [编码 Agent 三层安全门禁](#volc)：分三层拦截危险指令和注入攻击，误报 0/65
- [Agent 评测结论稳健性审计工具](#decisive)：换一个评测口径，你的结论还成立吗
- [电商对话式导购：反向用户建模检索](#converge)：不调用模型，平均 1.96 轮找准商品
- [中文开发者 API 报错安全诊断工作台](#debugcn)：先脱敏，再给出可复核的报错诊断
- [课程答辩 AI 陪练](#wbox)：答辩前找出自己没懂的地方，逼自己讲一遍

**[03 · 把金融规则与风险，做成产品机制](#fin)**

- [跨境支付反洗钱调查分诊方案](#crossborder)：同样人力下，产能内精确率 0.223 → 0.870
- [证券研究问答的证据与质检层](#wealthguard)：让每个金融回答都能追溯到带日期的官方原文
- [支付前反诈干预方案](#finsafe)：在付款前的最后几秒分级干预，AI 不能调低警示等级
- [风险分驱动的借贷条款设计](#defi)：把风险分翻译成借款人看得懂、能复算的借贷条款
- [外汇策略的研究—监控—执行产品化](#fx)：研究、监控、下单三块分开，生产环境直接拒绝
- [量化竞赛策略的反过拟合研究协议](#highbeta)：规则先冻结，负结果也照样公开
- [自适应反钓鱼培训 SaaS](#tbc)：面向个人和企业的反钓鱼培训，企业用户满意度 4.5/5


<!--
新项目放进最能说明它的那一组，照下面的模板写，并在上面的目录里加一行「- [方案名](#英文id)：一句话介绍」：
...
（三段放在 <details><summary>展开：背景与目标 · 我的职责 · 交付与成果</summary> … </details> 折叠块里，和其他项目一致）
-->

<a id="solve"></a>

## 01 / 把模糊的现场问题，落成能执行的方案

> 面对说不清的需求和不完整的信息，先诊断谁在做决定、卡在哪，再交付对方能照着做、能当场检查的方案：流程、工作簿、文书或系统。
...
把“一摞手写的陈年合同”理清成权属关系明确、可交律师复核的法律文书草案

<details>
<summary>展开：背景与目标 · 我的职责 · 交付与成果</summary>

- **背景与目标**：一个家族早年通过合作建房协议积累了多处物业权益，手里只有“一摞手写的陈年合同”：多份协议、申请记录和扫描件，不少是手写或残页；继承关系没有核实，也没有统一的管理主体。目标是把散乱的纸档理成一份每条都有出处、可交律师复核的权属判断和行动计划。
- **我的职责**：担任分析与统筹：收集原件，设计用 LLM 抽取和整理合同信息的流程，逐条对照原件核验 AI 的产出，决定每条结论的可信程度和行动顺序。法律意见、公证、税务和登记都交给持证专业人士。
- **交付与成果**：一条按地块区分的权属时间线；一套标注了可信程度的法律文书草案，包括权益确认协议；一份分工作线推进的行动计划，列出还缺哪些信息、有哪些风险、怎么应对。核验时拦下了 AI 把“拟议的分配”写成“已确认”的错误。目前是草案，尚未签署。
...
<p align="right"><a href="#nav">↑ 回到目录</a></p>

<a id="ai"></a>

## 02 / 给 AI 划清边界，并证明它可信

> 决定哪些事交给模型、哪些交给规则、哪些必须留给人；把权限写进代码而不是提示词，再用固定样本、消融和误报率证明它真的有用。
...
> 把合规要求、风险信号和交易纪律拆成状态、规则、阈值和审计记录，让资金相关的决定可解释、可复算、出错能追溯。
